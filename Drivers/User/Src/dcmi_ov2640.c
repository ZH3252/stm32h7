/***
   ...
   *  @description
	*
	*	实验平台：反客STM32H750XBH6核心板 （型号：FK750M5-XBH6）+ OV2640模块（型号：OV2640M1-200W） 
...
volatile uint8_t OV2640_FPS ;          // 帧率       // 帧率
...
void HAL_DCMI_MspInit(DCMI_HandleTypeDef* hdcmi)
{
   ...
   if(hdcmi->Instance==DCMI)
   {
		...
		__HAL_RCC_GPIOI_CLK_ENABLE();		// 使能相应的GPIO时钟
...
/****************************************************************************  
   数据引脚                       时钟和同步引脚
   PH9     ------> DCMI_D0        PI5     ------> DCMI_VSYNC
   PH10    ------> DCMI_D1        PH8     ------> DCMI_HSYNC
   PH11    ------> DCMI_D2        PA6  	------> DCMI_PIXCLK
   PH12    ------> DCMI_D3
   PH14    ------> DCMI_D4        SCCB 控制引脚，sccb.c 文件
   PI4     ------> DCMI_D5	       PI8  ------> SCCB_SCL
   PI6     ------> DCMI_D6        PI9  ------> SCCB_SDA 
   PI7     ------> DCMI_D7
